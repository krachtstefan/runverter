...
test('timeSec property can not be negative', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("timeSec", new BigNumber(-10)); });
  assert.strictEqual(runSubject.get("timeSec").toString(), "0");

  Ember.run(function(){ runSubject.set("timeHr", "-10"); });
  assert.strictEqual(runSubject.get("timeSec").toString(), "0");
});
...
test('timeHr setter changes timeSec', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("timeHr", "2.123"); });
...
test('timeMin setter changes timeSec', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeMin", "12"); });
  assert.strictEqual(runSubject.get("timeSec").toString(), "720");
  Ember.run(function(){ runSubject.set("timeMin", "12.123"); });
...
test('timeStackHr rounds properly', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("timeSec", new BigNumber(3600)); });
  ...
  Ember.run(function(){ runSubject.set("timeSec", new BigNumber(3599)); });
  ...
  Ember.run(function(){ runSubject.set("timeSec", new BigNumber(3599.5)); });
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "1"); // 0:59:59.5 will be rounded to 1:00:00
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "0");
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "0");

  Ember.run(function(){ runSubject.set("timeHr", new BigNumber(0.995)); });
...
test('timeStackHr setter changes timeStackHr', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackHr", "2"); });
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "2");
});

test('timeStackHr setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackHr", "2.2"); });
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "2");
  Ember.run(function(){ runSubject.set("timeStackHr", 2.5); });
...
test('timeStackHr setter influences all time related properties', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("timeStackHr", "2"); }); // 2,5 hours and 12 seconds
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "2");
...
test('timeStackMin rounds properly', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(14400), lengthM : new BigNumber(42195)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackMin", "5"); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "0"); // was -1 when calculation was based on raw values without round()

  Ember.run(function(){ runSubject.set("timeMin", new BigNumber("0.994")); });
  ...
  Ember.run(function(){ runSubject.set("timeMin", new BigNumber("0.991")); });
  ...
  Ember.run(function(){ runSubject.setProperties({ paceMinPerKmStackMin : "5", paceMinPerKmStackSec : "41" }); });
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "48");
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "59");
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "3");

  Ember.run(function(){ runSubject.set("timeSec", new BigNumber(3599.5)); }); // 0:59:59.5 will be rounded to 1:00:00
...
test('timeStackMin setter changes timeStackMin', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMin", "10"); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "10");
});

test('timeStackMin setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMin", "2.2"); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "2");
  Ember.run(function(){ runSubject.set("timeStackMin", 2.5); });
...
test('timeStackMin setter also works with integer', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMin", 2); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "2");
});

test('timeStackMin setter handles negative values and changes timeStackHr', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackHr", 5); });
  Ember.run(function(){ runSubject.set("timeStackMin", 55); });
  Ember.run(function(){ runSubject.set("timeStackMin", -10); });
...
test('timeStackMin setter handles values over 59 and changes timeStackHr', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackHr", 5); });
  Ember.run(function(){ runSubject.set("timeStackMin", 60); });
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "6");
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "0");
  Ember.run(function(){ runSubject.set("timeStackHr", 5); });
  Ember.run(function(){ runSubject.set("timeStackMin", 80); });
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "6");
...
test('timeStackMin setter influences all time related properties', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("timeStackMin", "10"); }); // 10 minutes, 30 seconds
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "0");
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "10");
...
test('timeStackMinFixed has leading zero(s)', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("timeSec", new BigNumber("2")); });
...
test('timeStackMinFixed setter also works with integer', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMinFixed", 20); });
...
test('timeStackMinFixed setter works with single digit', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMinFixed", "9"); });
...
test('timeStackMinFixed setter handles negative values and changes timeStackHr', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("timeHr", new BigNumber("6.5")); });
  Ember.run(function(){ runSubject.set("timeStackMinFixed", -40); });
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "5");
  assert.strictEqual(runSubject.get("timeStackMinFixed").toString(), "20");

  Ember.run(function(){ runSubject.set("timeHr", new BigNumber("5")); });
  Ember.run(function(){ runSubject.set("timeStackMinFixed", -1); });
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "4");
...
test('timeStackMinFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMinFixed", "09"); });
  assert.strictEqual(runSubject.get("timeStackMinFixed"), "09");
  Ember.run(function(){ runSubject.set("timeStackMinFixed", "002"); });
  assert.strictEqual(runSubject.get("timeStackMinFixed"), "02");
  Ember.run(function(){ runSubject.set("timeStackMinFixed", "009"); });
  assert.strictEqual(runSubject.get("timeStackMinFixed"), "09");
});

test('timeStackMinFixed setter influences all time related properties', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(90)}); // 1 minute, 30 seconds
  Ember.run(function(){ runSubject.set("timeStackMinFixed", "10"); }); // 10 minutes, 30 seconds
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "0");
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "10");
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "30");
  assert.strictEqual(runSubject.get("timeSec").toString(), "630");
...
test('timeStackSec setter changes timeStackSec', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackSec", "10"); });
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "10");
});

test('timeStackSec setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackSec", "2.2"); });
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "2");
  Ember.run(function(){ runSubject.set("timeStackSec", 2.5); });
...
test('timeStackSec setter also works with integer', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackSec", 2); });
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "2");
});

test('timeStackSec setter handles negative values and changes timeStackMin', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMin", 5); });
  Ember.run(function(){ runSubject.set("timeStackSec", 50); });
  Ember.run(function(){ runSubject.set("timeStackSec", -10); });
...
test('timeStackSec setter handles values over 59 and changes timeStackMin', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackMin", 5); });
  Ember.run(function(){ runSubject.set("timeStackSec", 60); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "0");
  Ember.run(function(){ runSubject.set("timeStackMin", 5); });
  Ember.run(function(){ runSubject.set("timeStackSec", 80); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "6");
...
test('timeStackSec setter influences all time related properties', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(12612)}); // 3,5 hours and 12 seconds
  Ember.run(function(){ runSubject.set("timeStackSec", "20"); }); // 3,5 hours and 20 seconds
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "3");
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "30");
  assert.strictEqual(runSubject.get("timeStackSec").toString(), "20");
...
test('timeStackSec setter handles values bigger than 59', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(12612)}); // 3,5 hours and 12 seconds
  Ember.run(function(){ runSubject.set("timeStackSec", "62"); }); // 3,5 hours and 62 seconds
  assert.strictEqual(runSubject.get("timeStackHr").toString(), "3");
...
test('timeStackSecFixed has leading zero(s)', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("timeSec", new BigNumber("60")); });
  assert.strictEqual(runSubject.get("timeStackSecFixed"), "00");
});
...
test('timeStackSecFixed setter also works with integer', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackSecFixed", 20); });
...
test('timeStackSecFixed setter works with single digit', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackSecFixed", "9"); });
...
test('timeStackSecFixed setter handles negative values and changes timeStackMin', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("timeMin", new BigNumber("6.5")); });
  Ember.run(function(){ runSubject.set("timeStackSecFixed", -40); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "5");
  assert.strictEqual(runSubject.get("timeStackSecFixed").toString(), "20");

  Ember.run(function(){ runSubject.set("timeMin", new BigNumber("5")); });
  Ember.run(function(){ runSubject.set("timeStackSecFixed", -1); });
  assert.strictEqual(runSubject.get("timeStackMin").toString(), "4");
...
test('timeStackSecFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("timeStackSecFixed", "09"); });
  assert.strictEqual(runSubject.get("timeStackSecFixed"), "09");
  Ember.run(function(){ runSubject.set("timeStackSecFixed", "002"); });
  assert.strictEqual(runSubject.get("timeStackSecFixed"), "02");
  Ember.run(function(){ runSubject.set("timeStackSecFixed", "009"); });
  assert.strictEqual(runSubject.get("timeStackSecFixed"), "09");
});
...
test('timeStackMin and timeStackSecFixed setter will define timeMin', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.setProperties({ timeStackMin : "6", timeStackSecFixed : "20" }); });
  assert.strictEqual(runSubject.get("timeMin").toString(), "6.33333333333333333333");
});
...
test('lengthM property can not be negative', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(-10)); });
  assert.strictEqual(runSubject.get("lengthM").toString(), "0");

  Ember.run(function(){ runSubject.set("lengthMi", "-10"); });
  assert.strictEqual(runSubject.get("lengthM").toString(), "0");
});
...
test('lengthMStackM property can be zero', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(0.3)); });
  assert.strictEqual(runSubject.get("lengthMStackM").toString(), "0");
});
...
test('lengthMStackM rounds properly', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(0.994)); });
...
test('lengthMStackDecimal setter handles negative values and changes lengthMStackM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("lengthMStackDecimal", -10); });
...
test('lengthMStackDecimal setter handles values over 99 and changes lengthMStackM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("lengthMStackDecimal", 100); });
  ...
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("lengthMStackDecimal", 123); });
  assert.strictEqual(runSubject.get("lengthMStackM").toString(), "5");
...
test('lengthMStackDecimal setter works with leading zeros', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMStackDecimal", "09"); });
  assert.strictEqual(runSubject.get("lengthMStackDecimal"), "09");
  Ember.run(function(){ runSubject.set("lengthMStackDecimal", "002"); });
  assert.strictEqual(runSubject.get("lengthMStackDecimal"), "0");
  Ember.run(function(){ runSubject.set("lengthMStackDecimal", "009"); });
...
test('lengthMStackM and lengthMStackDecimal setter will define lengthM', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.setProperties({ lengthMStackM : "12", lengthMStackDecimal : "09" }); });
...
test('lengthMStackDecimalFixed has trailing zero(s)', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5")); });
...
test('lengthMStackDecimalFixed supports leading zero', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(0.005)); });
...
test('lengthMStackDecimalFixed setter changes lengthMStackDecimalFixed', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", "90"); });
...
test('lengthMStackDecimalFixed setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", "8.2"); });
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed"), "80");
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", 9.5); });
...
test('lengthMStackDecimalFixed setter also works with integer', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", 9); });
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed"), "90");
});
...
test('lengthMStackDecimalFixed setter handles negative values and changes lengthMStackM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", -10); });
  assert.strictEqual(runSubject.get("lengthMStackM").toString(), "4");
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed").toString(), "90");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5.0")); });
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", -1); });
  assert.strictEqual(runSubject.get("lengthMStackM").toString(), "4");
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed").toString(), "99");
});

test('lengthMStackDecimalFixed setter handles values over 99 and changes lengthMStackM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", 100); });
  assert.strictEqual(runSubject.get("lengthMStackM").toString(), "6");
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed").toString(), "00");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", 123); });
  assert.strictEqual(runSubject.get("lengthMStackM").toString(), "6");
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed").toString(), "23");
});

test('lengthMStackDecimalFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", "09"); });
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed"), "09");
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", "002"); });
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed"), "00");
  Ember.run(function(){ runSubject.set("lengthMStackDecimalFixed", "009"); });
  assert.strictEqual(runSubject.get("lengthMStackDecimalFixed"), "01");
});
...
test('lengthMStackM and lengthMStackDecimalFixed setter will define lengthM', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.setProperties({ lengthMStackM : "12", lengthMStackDecimalFixed : "9" }); });
...
test('lengthKm can have up to 20 decimal places and can round up', function(assert) {
  ...
  var runSubject = this.subject({lengthM : new BigNumber("9.12345678901234").dividedBy(6)});
  // http://keisan.casio.com/calculator results in 0.0015205761315020566666666667
  assert.strictEqual(runSubject.get("lengthKm").toString(), "0.00152057613150205667");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("8.78748598595").dividedBy(3)); });
  // http://keisan.casio.com/calculator results in 0.0029291619953166666666666667
  assert.strictEqual(runSubject.get("lengthKm").toString(), "0.00292916199531666667");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("8.123455").dividedBy(7)); });
  // http://keisan.casio.com/calculator results in 0.0011604935714285714285714286
  assert.strictEqual(runSubject.get("lengthKm").toString(), "0.00116049357142857143");
});

test('lengthKm can round down', function(assert) {
  // BigNumber can only be initialized with 15 digit, dividing it will result in more digits
  var runSubject = this.subject({lengthM : new BigNumber("9.12345678901234").dividedBy(9)});
  // http://keisan.casio.com/calculator results in 0.001013717421001371111111

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("9.3343442341212212212").dividedBy(5)); });
  // http://keisan.casio.com/calculator results in 0.00186686884682424424424
  assert.strictEqual(runSubject.get("lengthKm").toString(), "0.00186686884682424424");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber("9.3343442341214223423324212").dividedBy(3)); });
  // http://keisan.casio.com/calculator results in 0.0031114480780404741141108071
  assert.strictEqual(runSubject.get("lengthKm").toString(), "0.00311144807804047411");
});

test('lengthKm setter changes lengthKm', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKm", "100"); });
  assert.strictEqual(runSubject.get("lengthKm").toString(), "100");
});

test('lengthKm setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKm", "100.12345"); });
  assert.strictEqual(runSubject.get("lengthKm").toString(), "100.12345");
  Ember.run(function(){ runSubject.set("lengthKm", 100.34); });
...
test('lengthKm setter changes lengthM', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKm", "12"); });
  assert.strictEqual(runSubject.get("lengthM").toString(), "12000");
  Ember.run(function(){ runSubject.set("lengthKm", "12.123"); });
...
test('lengthKmStackKm rounds properly', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthKm", new BigNumber(0.995)); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "1"); // the lengthKm of 0.995 results in a lengthKmStackKm of 1

  Ember.run(function(){ runSubject.set("lengthKm", new BigNumber(0.994)); });
...
test('lengthKmStackKm setter changes lengthKmStackKm', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackKm", "2"); });
...
test('lengthKmStackKm setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackKm", "2.9"); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "3");
  Ember.run(function(){ runSubject.set("lengthKmStackKm", 2.3); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "2");
});
...
test('lengthKmStackDecimal setter changes lengthKmStackDecimal', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", "9"); });
...
test('lengthKmStackDecimal setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", "8.2"); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimal"), "8");
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", 9.5); });
...
test('lengthKmStackDecimal setter handles negative values and changes lengthKmStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthKm", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", -10); });
...
test('lengthKmStackDecimal setter handles values over 99 and changes lengthKmStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthKm", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", 100); });
  ...
  Ember.run(function(){ runSubject.set("lengthKm", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", 123); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "5");
...
test('lengthKmStackDecimal setter works with leading zeros', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", "09"); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimal"), "09");
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", "002"); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimal"), "0");
  Ember.run(function(){ runSubject.set("lengthKmStackDecimal", "009"); });
...
test('lengthKmStackKm and lengthKmStackDecimal setter will define lengthKm', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.setProperties({ lengthKmStackKm : "12", lengthKmStackDecimal : "09" }); });
...
test('lengthKmStackDecimalFixed has trailing zero(s)', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(2)); });
...
test('lengthKmStackDecimalFixed setter changes lengthKmStackDecimalFixed', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", "90"); });
...
test('lengthKmStackDecimalFixed setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", "8.2"); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed"), "80");
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", 9.5); });
...
test('lengthKmStackDecimalFixed setter also works with integer', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", 9); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed"), "90");
});
...
test('lengthKmStackDecimalFixed setter handles negative values and changes lengthKmStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthKm", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", -10); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "4");
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed").toString(), "90");

  Ember.run(function(){ runSubject.set("lengthKm", "5.0"); });
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", -1); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "4");
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed").toString(), "99");
});

test('lengthKmStackDecimalFixed setter handles values over 99 and changes lengthKmStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthKm", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", 100); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "6");
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed").toString(), "00");

  Ember.run(function(){ runSubject.set("lengthKm", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", 123); });
  assert.strictEqual(runSubject.get("lengthKmStackKm").toString(), "6");
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed").toString(), "23");
});

test('lengthKmStackDecimalFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", "09"); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed"), "09");
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", "002"); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed"), "00");
  Ember.run(function(){ runSubject.set("lengthKmStackDecimalFixed", "009"); });
  assert.strictEqual(runSubject.get("lengthKmStackDecimalFixed"), "01");
});
...
test('lengthKmStackKm and lengthKmStackDecimalFixed setter will define lengthKm', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.setProperties({ lengthKmStackKm : "12", lengthKmStackDecimalFixed : "9" }); });
...
test('lengthMi can have up to 20 decimal places and can round up', function(assert) {
  ...
  assert.strictEqual(runSubject.get("lengthMi").toString(), "0.00566905322231440264");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(14232.25)); });
  // http://keisan.casio.com/calculator results in 8.8435101507197963890877277
  assert.strictEqual(runSubject.get("lengthMi").toString(), "8.84351015071979638909");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(123.45)); });
  // http://keisan.casio.com/calculator results in 0.07670827368169887854927225
  assert.strictEqual(runSubject.get("lengthMi").toString(), "0.07670827368169887855");
});
...
test('lengthMi can round down', function(assert) {
  ...
  assert.strictEqual(runSubject.get("lengthMi").toString(), "0.00566905322231439642");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(123)); });
  // http://keisan.casio.com/calculator results in 0.076428656645192078262944405
  assert.strictEqual(runSubject.get("lengthMi").toString(), "0.07642865664519207826");

  Ember.run(function(){ runSubject.set("lengthM", new BigNumber(12.12)); });
  // http://keisan.casio.com/calculator results in 0.0075310188499164877117633023
  assert.strictEqual(runSubject.get("lengthMi").toString(), "0.00753101884991648771");
});

test('lengthMi setter changes lengthMi', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMi", "100"); });
  assert.strictEqual(runSubject.get("lengthMi").toString(), "100");
});

test('lengthMi setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMi", "100.55"); });
  assert.strictEqual(runSubject.get("lengthMi").toString(), "100.55");
  Ember.run(function(){ runSubject.set("lengthMi", 100.12); });
...
test('lengthMi setter changes lengthM', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMi", "12"); });
  assert.strictEqual(runSubject.get("lengthM").toString(), "19312.128");
  Ember.run(function(){ runSubject.set("lengthMi", "12.123"); });
...
test('lengthMiStackMi rounds properly', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthMi", new BigNumber(0.995)); });
  assert.strictEqual(runSubject.get("lengthMiStackMi").toString(), "1"); // the lengthMi of 0.995 results in a lengthMiStackMi of 1

  Ember.run(function(){ runSubject.set("lengthMi", new BigNumber(0.994)); });
...
test('lengthMiStackMi setter changes lengthMiStackMi', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackMi", "2"); });
...
test('lengthMiStackMi setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackMi", "2.2"); });
  assert.strictEqual(runSubject.get("lengthMiStackMi").toString(), "2");
  Ember.run(function(){ runSubject.set("lengthMiStackMi", 2.5); });
...
test('lengthMiStackDecimal setter changes lengthMiStackDecimal', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", "9"); });
...
test('lengthMiStackDecimal setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", 9.2); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimal"), "9");
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", "9.5"); });
...
test('lengthMiStackDecimal setter handles negative values and changes lengthMiStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthMi", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", -10); });
...
test('lengthMiStackDecimal setter handles values over 99 and changes lengthMiStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthMi", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", 100); });
  ...
  Ember.run(function(){ runSubject.set("lengthMi", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", 123); });
  assert.strictEqual(runSubject.get("lengthMiStackMi").toString(), "5");
...
test('lengthMiStackDecimal setter works with leading zeros', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", "09"); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimal"), "09");
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", "002"); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimal"), "0");
  Ember.run(function(){ runSubject.set("lengthMiStackDecimal", "009"); });
...
test('lengthMiStackDecimal and lengthMiStackDecimal setter will define lengthMi', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.setProperties({ lengthMiStackMi : "12", lengthMiStackDecimal : "09" }); });
...
test('lengthMiStackDecimalFixed has trailing zero(s)', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthMi", new BigNumber(14)); });
...
test('lengthMiStackDecimalFixed supports leading zero', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("lengthMi", new BigNumber(12.005)); });
...
test('lengthMiStackDecimalFixed setter changes lengthMiStackDecimalFixed', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", "90"); });
...
test('lengthMiStackDecimalFixed setter can handle floats', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", "8.2"); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed"), "80");
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", 9.5); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed"), "10");
});

test('lengthMiStackDecimalFixed setter also works with integer', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", 9); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed"), "90");
});
...
test('lengthMiStackDecimalFixed setter handles negative values and changes lengthMiStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthMi", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", -10); });
  assert.strictEqual(runSubject.get("lengthMiStackMi").toString(), "4");
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed").toString(), "90");

  Ember.run(function(){ runSubject.set("lengthMi", "5.0"); });
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", -1); });
  assert.strictEqual(runSubject.get("lengthMiStackMi").toString(), "4");
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed").toString(), "99");
});

test('lengthMiStackDecimalFixed setter handles values over 99 and changes lengthMiStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("lengthMi", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", 100); });
  assert.strictEqual(runSubject.get("lengthMiStackMi").toString(), "6");
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed").toString(), "00");

  Ember.run(function(){ runSubject.set("lengthMi", "5.5"); });
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", 123); });
  assert.strictEqual(runSubject.get("lengthMiStackMi").toString(), "6");
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed").toString(), "23");
});

test('lengthMiStackDecimalFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", "09"); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed"), "09");
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", "002"); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed"), "00");
  Ember.run(function(){ runSubject.set("lengthMiStackDecimalFixed", "009"); });
  assert.strictEqual(runSubject.get("lengthMiStackDecimalFixed"), "01");
});
...
test('lengthMiStackMi and lengthMiStackDecimalFixed setter will define lengthMi', function(assert) {
  var runSubject = this.subject();
  Ember.run(function(){ runSubject.setProperties({ lengthMiStackMi : "12", lengthMiStackDecimalFixed : "9" }); });
...
test('paceMinPerKm property is calculated from timeSec and lengthM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(60), lengthM : new BigNumber(1000)});
...
test('paceMinPerKm can have up to 20 decimal places and can round up', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.1234567891238), lengthM : new BigNumber(900)});
  // http://keisan.casio.com/calculator results in 1.2482853212486666666666667
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "1.24828532124866666667");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(2.9090), lengthM : new BigNumber(93) }); });
  // http://keisan.casio.com/calculator results in 31.279569892473118279569892
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "31.27956989247311827957");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(2.9090), lengthM : new BigNumber(123) }); });
  // http://keisan.casio.com/calculator results in 23.650406504065040650406504
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "23.65040650406504065041");
});

test('paceMinPerKm can round down', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.1234567891234), lengthM : new BigNumber(900)});
  // http://keisan.casio.com/calculator results in 1.248285321248222222222
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "1.24828532124822222222");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(2.1234567891238), lengthM : new BigNumber(921) }); });
  // http://keisan.casio.com/calculator results in 2.3055991195698154180238871
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "2.30559911956981541802");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(2.12345678238), lengthM : new BigNumber(123) }); });
  // http://keisan.casio.com/calculator results in 17.26387627951219512195122
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "17.26387627951219512195");
});

test('paceMinPerKm is not getting infinite when length is zero', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(30), lengthM : new BigNumber(0)});
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "0");
});

test('paceMinPerKm setter changes paceMinPerKm', function(assert) {
  var runSubject = this.subject({lengthM : new BigNumber(2000)});
  Ember.run(function(){ runSubject.set("paceMinPerKm", "21"); });
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "21");
});

test('paceMinPerKm setter can handle floats', function(assert) {
  var runSubject = this.subject({lengthM : new BigNumber(2000)});
  Ember.run(function(){ runSubject.set("paceMinPerKm", 2.2); });
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "2.2");
  Ember.run(function(){ runSubject.set("paceMinPerKm", "2.5"); });
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "2.5");
  Ember.run(function(){ runSubject.set("paceMinPerKm", 2.21234); });
...
test('paceMinPerKm setter changes timeSec', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("paceMinPerKm", "2"); });
...
test('paceMinPerKmStackMin property is calculated from lengthM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.23454), lengthM : new BigNumber(1000)});
...
test('paceMinPerKmStackMin property can be zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1500)});
...
test('paceMinPerKmStackMin rounds properly', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1)});

  Ember.run(function(){ runSubject.set("paceMinPerKm", new BigNumber(0.995)); });
  ...
  Ember.run(function(){ runSubject.set("paceMinPerKm", new BigNumber(0.994)); });
...
test('paceMinPerKmStackMin setter changes paceMinPerKmStackMin', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(270), lengthM : new BigNumber(2000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackMin", "12"); });
...
test('paceMinPerKmStackMin setter can handle floats', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(270), lengthM : new BigNumber(2000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackMin", "5.5"); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "6");
  Ember.run(function(){ runSubject.set("paceMinPerKmStackMin", 2.3); });
...
test('paceMinPerKmStackMin setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(60), lengthM : new BigNumber(2000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackMin", "2"); });
...
test('paceMinPerKmStackSec property is calculated from timeMin and lengthM and can round down' , function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.54), lengthM : new BigNumber(1000)});
...
test('paceMinPerKmStackSec property can round up' , function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.56), lengthM : new BigNumber(1000)});
...
test('paceMinPerKmStackSec rounds properly', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(2999.9999999999), lengthM : new BigNumber(10000)});
...
test('paceMinPerKmStackSec setter changes paceMinPerKmStackSec', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", "10"); });
...
test('paceMinPerKmStackSec setter can handle floats', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", "2.2"); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSec").toString(), "2");
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", 2.5); });
...
test('paceMinPerKmStackSec setter also works with integer', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", 2); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSec").toString(), "2");
});

test('paceMinPerKmStackSec setter handles negative values and changes paceMinPerKmStackMin', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKm", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", -10); });
...
test('paceMinPerKmStackSec setter handles values over 59 and changes paceMinPerKmStackMin', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKm", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", 60); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerKmStackSec").toString(), "0");
  Ember.run(function(){ runSubject.set("paceMinPerKm", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", 80); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "6");
...
test('paceMinPerKmStackSec setter influences all pace related properties', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(12), lengthM : new BigNumber(2000)}); // 6 min/km
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", "20"); }); // 6.3333 min/km
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerKmStackSec").toString(), "20");
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "6.33333333333333333333");
});

test('paceMinPerKmStackSec setter handles values bigger than 59', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(12), lengthM : new BigNumber(2000)}); // 6 min/km
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", "90"); }); // 7.5000 min/km
...
test('paceMinPerKmStackSec setter should refer to an uncompressed version paceMinPerKmStackSec (with digits) to have exact results', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600*4), lengthM : new BigNumber(42195)});
  ...
  assert.strictEqual(runSubject.get("paceMinPerKm").toString(), "5.68787771062922147174");
  ...
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSec", "0"); });
...
test('paceMinPerKmStackSecFixed property property is calculated from timeMin and lengthM and can round down', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.54), lengthM : new BigNumber(1000)});
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "32"); // 60 * 0,54 = 32,4 seconds
});

test('paceMinPerKmStackSecFixed property can round up', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.56), lengthM : new BigNumber(1000)});
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "34"); // 60 * 0,56 = 33,6 seconds
});

test('paceMinPerKmStackSecFixed has leading zero(s)', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(75.083), lengthM : new BigNumber(1000)});
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed"), "05");
  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(75), lengthM : new BigNumber(1000) }); });
...
test('paceMinPerKmStackSecFixed setter changes paceMinPerKmStackSecFixed', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", "10"); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "10");
});

test('paceMinPerKmStackSecFixed setter can handle floats', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", "2.2"); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "02");
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", 2.5); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "03");
});

test('paceMinPerKmStackSecFixed setter also works with integer', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", 12); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "12");
});

test('paceMinPerKmStackSecFixed setter works with single digit', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", 2); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "02");
});
...
test('paceMinPerKmStackSecFixed setter handles negative values and changes paceMinPerKmStackMin', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", -10); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "4");
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "50");

  Ember.run(function(){ runSubject.set("paceMinPerKm", "5"); });
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", -1); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "4");
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "59");
});

test('paceMinPerKmStackSecFixed setter handles values over 59 and changes paceMinPerKmStackMin', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKm", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", 60); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "00");

  Ember.run(function(){ runSubject.set("paceMinPerKm", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", 80); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed").toString(), "20");
});

test('paceMinPerKmStackSecFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", "09"); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed"), "09");
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", "002"); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed"), "02");
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", "009"); });
  assert.strictEqual(runSubject.get("paceMinPerKmStackSecFixed"), "09");
});

test('paceMinPerKmStackSecFixed setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerKmStackSecFixed", "30"); });
...
test('paceMinPerKmStackMin and paceMinPerKmStackSecFixed setter will define paceMinPerKm', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ paceMinPerKmStackMin : "5", paceMinPerKmStackSecFixed : "30" }); });
...
test('paceMinPerMi property is calculated from timeSec and lengthM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(60), lengthM : new BigNumber(1609.344)});
...
test('paceMinPerMi can have up to 20 decimal places and can round up', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.1234567891238), lengthMi : new BigNumber(900)});
  // http://keisan.casio.com/calculator results in 0.0012482853212486666666666667
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "0.00124828532124866667");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(1.1234567891238), lengthMi : new BigNumber(123) }); });
  // http://keisan.casio.com/calculator results in 0.001248285321248666666667
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "0.00913379503352682927");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(13), lengthMi : new BigNumber(124) }); });
  // http://keisan.casio.com/calculator results in 0.10483870967741935483870968
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "0.10483870967741935484");
});

test('paceMinPerMi can round down', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(3.1131567891211), lengthMi : new BigNumber(900)});
  // http://keisan.casio.com/calculator results in 0.0034590630990234444444444444
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "0.00345906309902344444");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(9), lengthMi : new BigNumber(124) }); });
  // http://keisan.casio.com/calculator results in 0.072580645161290322580645161
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "0.07258064516129032258");

  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(1245), lengthMi : new BigNumber(124) }); });
  // http://keisan.casio.com/calculator results in 10.04032258064516129032 2581
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "10.04032258064516129032");
});
...
test('paceMinPerMi setter changes timeSec', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("paceMinPerMi", "2"); });
...
test('paceMinPerMiStackMin property is calculated from lengthM', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.23454), lengthMi : new BigNumber(1)});
...
test('paceMinPerMiStackMin property can be zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthMi : new BigNumber(1.500)});
...
test('paceMinPerMiStackMin rounds properly', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthMi : new BigNumber(1)});

  Ember.run(function(){ runSubject.set("paceMinPerMi", new BigNumber(0.995)); });
  ...
  Ember.run(function(){ runSubject.set("paceMinPerMi", new BigNumber(0.994)); });
...
test('paceMinPerMiStackMin setter changes paceMinPerMiStackMin', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(270), lengthMi : new BigNumber(2)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackMin", "12"); });
...
test('paceMinPerMiStackMin setter can handle floats', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(270), lengthMi : new BigNumber(2)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackMin", "5.5"); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "6");
  Ember.run(function(){ runSubject.set("paceMinPerMiStackMin", 2.3); });
...
test('paceMinPerMiStackMin setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(60), lengthMi : new BigNumber(2)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackMin", "2"); });
...
test('paceMinPerMiStackSec property is calculated from timeMin and lengthM and can round down' , function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.54), lengthM : new BigNumber(1609.344)});
...
test('paceMinPerMiStackSec property can round up' , function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.56), lengthM : new BigNumber(1609.344)});
...
test('paceMinPerMiStackSec rounds properly', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(1864), lengthM : new BigNumber(10000)});
...
test('paceMinPerMiStackSec setter changes paceMinPerMiStackSec', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthMi : new BigNumber(1)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", "10"); });
...
test('paceMinPerMiStackSec setter can handle floats', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthMi : new BigNumber(1)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", "2.2"); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSec").toString(), "2");
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", 2.5); });
...
test('paceMinPerMiStackSec setter also works with integer', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", 2); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSec").toString(), "2");
});

test('paceMinPerMiStackSec setter handles negative values and changes paceMinPerMiStackMin', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMi", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", -10); });
...
test('paceMinPerMiStackSec setter handles values over 59 and changes paceMinPerMiStackMin', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMi", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", 60); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerMiStackSec").toString(), "0");
  Ember.run(function(){ runSubject.set("paceMinPerMi", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", 80); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "6");
...
test('paceMinPerMiStackSec setter influences all pace related properties', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(12), lengthMi : new BigNumber(2)});  // 6 min/mi
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", "20"); }); // 6.3333 min/km
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerMiStackSec").toString(), "20");
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "6.33333333333333333333");
});

test('paceMinPerMiStackSec setter handles values bigger than 59', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(12), lengthMi : new BigNumber(2)});  // 6 min/mi
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", "90"); }); // 7.5 min/km
...
test('paceMinPerMiStackSec setter should refer to an uncompressed version paceMinPerMiStackSec (with digits) to have exact results', function(assert) {
  ...
  assert.strictEqual(runSubject.get("paceMinPerMi").toString(), "9.15375186633487380021");
  ...
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSec", "0"); });
...
test('paceMinPerMiStackSecFixed property property is calculated from timeMin and lengthM and can round down', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.54), lengthM : new BigNumber(1609.344)});
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "32"); // 60 * 0,54 = 32,4 seconds
});

test('paceMinPerMiStackSecFixed property can round up', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.56), lengthM : new BigNumber(1609.344)});
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "34"); // 60 * 0,56 = 33,6 seconds
});

test('paceMinPerMiStackSecFixed has leading zero(s)', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(75.083), lengthMi : new BigNumber(1)});
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed"), "05");
  Ember.run(function(){ runSubject.setProperties({ timeMin : new BigNumber(75), lengthMi : new BigNumber(1) }); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed"), "00");
});

test('paceMinPerMiStackSecFixed can be zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(60), lengthMi : new BigNumber(1)});
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed"), "00");
});

test('paceMinPerMiStackSecFixed setter changes paceMinPerMiStackSecFixed', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", "10"); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "10");
});

test('paceMinPerMiStackSecFixed setter can handle floats', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", "2.2"); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "02");
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", 2.5); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "03");
});

test('paceMinPerMiStackSecFixed setter also works with integer', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", 12); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "12");
});

test('paceMinPerMiStackSecFixed setter works with single digit', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1.5), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", 2); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "02");
});
...
test('paceMinPerMiStackSecFixed setter handles negative values and changes paceMinPerMiStackMin', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", -10); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "4");
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "50");

  Ember.run(function(){ runSubject.set("paceMinPerMi", "5"); });
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", -1); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "4");
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "59");
});

test('paceMinPerMiStackSecFixed setter handles values over 59 and changes paceMinPerMiStackMin', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthMi : new BigNumber(1)});
  Ember.run(function(){ runSubject.set("paceMinPerMi", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", 60); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "00");

  Ember.run(function(){ runSubject.set("paceMinPerMi", "5.5"); });
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", 80); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackMin").toString(), "6");
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed").toString(), "20");
});

test('paceMinPerMiStackSecFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", "09"); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed"), "09");
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", "002"); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed"), "02");
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", "009"); });
  assert.strictEqual(runSubject.get("paceMinPerMiStackSecFixed"), "09");
});

test('paceMinPerMiStackSecFixed setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthMi : new BigNumber(1)});
  Ember.run(function(){ runSubject.set("paceMinPerMiStackSecFixed", "30"); });
  assert.strictEqual(runSubject.get("timeSec").toString(), "3630");
});

test('paceMinPerMiStackMin and paceMinPerMiStackSecFixed setter will define paceMinPerMi', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthMi : new BigNumber(1)});
  Ember.run(function(){ runSubject.setProperties({ paceMinPerMiStackMin : "5", paceMinPerMiStackSecFixed : "30" }); });
...
test('speedKmHr property is calculated from timeSec and lengthM', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(1500)});
...
test('speedKmHr can have up to 20 decimal places and can round up', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(11), lengthM : new BigNumber(23.4511)});
  // http://keisan.casio.com/calculator results in 7.6749054545454545454545455
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "7.67490545454545454545");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(11), lengthM : new BigNumber(23.4510) }); });
  // http://keisan.casio.com/calculator results in 7.6748727272727272727272727
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "7.67487272727272727273");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(11), lengthM : new BigNumber(12.121) }); });
  // http://keisan.casio.com/calculator results in 3.9668727272727272727272727
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "3.96687272727272727273");
});

test('speedKmHr can round down', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(49), lengthM : new BigNumber(12.9912)});
  // http://keisan.casio.com/calculator results in 0.9544555102040816326531
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "0.95445551020408163265");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(11), lengthM : new BigNumber(12.9912) }); });
  // http://keisan.casio.com/calculator results in 4.2516654545454545454545455
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "4.25166545454545454545");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(11), lengthM : new BigNumber(23.9912) }); });
  // http://keisan.casio.com/calculator results in 7.8516654545454545454545455
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "7.85166545454545454545");
});

test('speedKmHr is not getting infinite when time is zero', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(0), lengthM : new BigNumber(123456)});
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "0");
});

test('speedKmHr setter changes speedKmHr', function(assert) {
  var runSubject = this.subject({lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHr", "21"); });
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "21");
});

test('speedKmHr setter can handle floats', function(assert) {
  var runSubject = this.subject({lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHr", 2.2); });
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "2.2");
  Ember.run(function(){ runSubject.set("speedKmHr", "2.5"); });
  assert.strictEqual(runSubject.get("speedKmHr").toString(), "2.5");
  Ember.run(function(){ runSubject.set("speedKmHr", 2.21234); });
...
test('speedKmHr setter also works with integer', function(assert) {
  var runSubject = this.subject({lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHr", 2); });
...
test('speedKmHr setter doesn\'t change lengthM', function(assert) {
  ...
  Ember.run(function(){ runSubject.set("speedKmHr", "12"); });
...
test('speedKmHrStackKm property is calculated from timeSec and lengthM', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(25000)});
...
test('speedKmHrStackKm property can be zero', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(1)});

  Ember.run(function(){ runSubject.set("speedKmHr", new BigNumber(0.995)); }); // the speedKmHr of 0.995 results in a speedKmHrStackKm of 1
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "1");

  Ember.run(function(){ runSubject.set("speedKmHr", new BigNumber(0.994)); }); // the speedKmHr of 0.994 results in a speedKmHrStackKm of 0
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "0");
});

test('speedKmHrStackKm setter changes speedKmHrStackKm', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(24000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackKm", "18"); });
...
test('speedKmHrStackKm setter can handle floats', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(24000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackKm", "5.5"); });
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "6");
  Ember.run(function(){ runSubject.set("speedKmHrStackKm", 2.3); });
...
test('speedKmHrStackKm setter also works with integer', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(24123)});
  Ember.run(function(){ runSubject.set("speedKmHrStackKm", 12); });
...
test('speedKmHrStackKm setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(8000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackKm", "2"); });
  assert.strictEqual(runSubject.get("timeSec").toString(), "14400"); // 8km with 2km/hr will take 4 hours (14400 sek)
});

test('speedKmHrStackKm setter doesn\'t change lengthM', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(2500)});
  Ember.run(function(){ runSubject.set("speedKmHrStackKm", "12"); });
  assert.strictEqual(runSubject.get("lengthM").toString(), "2500");
});
...
test('speedKmHrStackDecimal property is calculated from timeSec and lengthM and can round down', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(25123)});
...
test('speedKmHrStackDecimal property can round up', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(25125)});
  assert.strictEqual(runSubject.get("speedKmHrStackDecimal"), "13");
});

test('speedKmHrStackDecimal can have 1 digit', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12800)});
...
test('speedKmHrStackDecimal supports leading zero', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12012)});
  assert.strictEqual(runSubject.get("speedKmHrStackDecimal"), "01");
});

test('speedKmHrStackDecimal can be zero', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12000)});
...
test('speedKmHrStackDecimal setter changes speedKmHrStackDecimal', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", "9"); });
...
test('speedKmHrStackDecimal setter can handle floats', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", "8.2"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimal"), "8");
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", 8.5); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimal"), "9");
});
...
test('speedKmHrStackDecimal setter handles negative values and changes speedKmHrStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHr", "5.5"); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", -10); });
...
test('speedKmHrStackDecimal setter handles values over 99 and changes speedKmHrStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHr", "5.5"); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", 100); });
  ...
  Ember.run(function(){ runSubject.set("speedKmHr", "5.5"); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", 123); });
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "5");
...
test('speedKmHrStackDecimal setter works with leading zeros', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", "09"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimal"), "09");
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", "002"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimal"), "0");
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimal", "009"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimal"), "01");
});

test('speedKmHrStackDecimal setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(3000)});
  Ember.run(function(){ runSubject.setProperties({ speedKmHrStackKm : "1", speedKmHrStackDecimal : "5" }); });
...
test('speedKmHrStackKm and speedKmHrStackDecimal setter will define speedKmHr', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(4400)});
  Ember.run(function(){ runSubject.setProperties({ speedKmHrStackKm : "12", speedKmHrStackDecimal : "05" }); });
...
test('speedKmHrStackDecimalFixed property is calculated from speedKmHr and can round down', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedKmHr : new BigNumber(12.121) }); });
...
test('speedKmHrStackDecimalFixed property can round up', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedKmHr : new BigNumber(12.715) }); });
...
test('speedKmHrStackDecimalFixed has trailing zero(s)', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedKmHr : new BigNumber(0.5) }); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "50");
  Ember.run(function(){ runSubject.setProperties({ speedKmHr : new BigNumber(5) }); });
...
test('speedKmHrStackDecimalFixed supports leading zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedKmHr : new BigNumber(0.09) }); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "09");
  Ember.run(function(){ runSubject.setProperties({ speedKmHr : new BigNumber(0.005) }); });
...
test('speedKmHrStackDecimalFixed can be zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedKmHr : new BigNumber(99) }); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "00");
});

test('speedKmHrStackDecimalFixed setter changes speedKmHrStackDecimalFixed', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", "90"); });
...
test('speedKmHrStackDecimalFixed setter can handle floats', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", "8.2"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "80");
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", 9.5); });
...
test('speedKmHrStackDecimalFixed setter also works with integer', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", 9); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "90");
});
...
test('speedKmHrStackDecimalFixed setter handles negative values and changes speedKmHrStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("speedKmHr", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", -10); });
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "4");
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed").toString(), "90");

  Ember.run(function(){ runSubject.set("speedKmHr", new BigNumber("5.0")); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", -1); });
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "4");
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed").toString(), "99");
});

test('speedKmHrStackDecimalFixed setter handles values over 99 and changes speedKmHrStackKm', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("speedKmHr", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", 100); });
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "6");
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed").toString(), "00");

  Ember.run(function(){ runSubject.set("speedKmHr", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", 123); });
  assert.strictEqual(runSubject.get("speedKmHrStackKm").toString(), "6");
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed").toString(), "23");
});

test('speedKmHrStackDecimalFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", "09"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "09");
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", "002"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "00");
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", "009"); });
  assert.strictEqual(runSubject.get("speedKmHrStackDecimalFixed"), "01");
});

test('speedKmHrStackDecimalFixed setter changes speedKmHr', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("speedKmHr", "1000"); });
  Ember.run(function(){ runSubject.set("speedKmHrStackDecimalFixed", "09"); });
...
test('speedKmHrStackKm and speedKmHrStackDecimalFixed setter will define speedKmHr', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.setProperties({ speedKmHrStackKm : "12", speedKmHrStackDecimalFixed : "9" }); });
...
test('speedMiHr property is calculated from timeSec and lengthM', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1609.344)});
...
test('speedMiHr can have up to 20 decimal places and can round up', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1629.364239)});
  // http://keisan.casio.com/calculator results in 1.0124399997763063707945598
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "1.01243999977630637079");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(3600), lengthM : new BigNumber(1629.380333) }); });
  // http://keisan.casio.com/calculator results in 1.0124500001242742384474668
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "1.01245000012427423845");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(3600), lengthM : new BigNumber(1629.380322) }); });
  // http://keisan.casio.com/calculator results in 1.0124499932891911238367931
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "1.01244999328919112384");
});

test('speedMiHr property can round up', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1629.380999)});
  // http://keisan.casio.com/calculator results in 1.0124504139574882685118906
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "1.01245041395748826851");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(3600), lengthM : new BigNumber(1721) }); });
  // http://keisan.casio.com/calculator results in 1.0693798218404517617116042
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "1.06937982184045176171");

  Ember.run(function(){ runSubject.setProperties({ timeSec : new BigNumber(3600), lengthM : new BigNumber(12345.12) }); });
  // http://keisan.casio.com/calculator results in 7.6709019327129563350035791
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "7.670901932712956335"); // two zeros at the end are falling away
});
...
test('speedMiHr setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(6437.376)});
  Ember.run(function(){ runSubject.set("speedMiHr", "2"); });
...
test('speedMiHr setter doesn\'t change lengthM', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(6437.376)});
  Ember.run(function(){ runSubject.set("speedMiHr", "12"); });
...
test('speedMiHrStackMi property is calculated from timeSec and lengthM', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(4000)});
...
test('speedMiHrStackMi rounds properly', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1)});

  Ember.run(function(){ runSubject.set("speedMiHr", new BigNumber(0.995)); }); // a speedMiHr of 0.995 results in a speedMiHrStackMi of 1
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "1");

  Ember.run(function(){ runSubject.set("speedMiHr", new BigNumber(0.994)); }); // a speedMiHr of 0.994 results in a speedMiHrStackMi of 0
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "0");
});

test('speedMiHrStackMi setter changes speedMiHrStackMi', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(24000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackMi", "18"); });
...
test('speedMiHrStackMi setter can handle floats', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(24000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackMi", "5.5"); });
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "6");
  Ember.run(function(){ runSubject.set("speedMiHrStackMi", 2.3); });
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "2");
});

test('speedMiHrStackMi setter also works with integer', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(24123)});
  Ember.run(function(){ runSubject.set("speedMiHrStackMi", 12); });
...
test('speedMiHrStackMi setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(6437.376)});
  Ember.run(function(){ runSubject.set("speedMiHrStackMi", "4"); });
...
test('speedMiHrStackDecimal property is calculated from timeSec and lengthM and can round down', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(2500.920576)}); // 1.554 mi
...
test('speedMiHrStackDecimal property can round up', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(2502.52992)}); // 1.555 mi
  assert.strictEqual(runSubject.get("speedMiHrStackDecimal"), "56");
});

test('speedMiHrStackDecimal can have 1 digit', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(2414.02)}); // 1.5 mi
...
test('speedMiHrStackDecimal supports leading zero', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1689.811)}); // 1.05 mi
  assert.strictEqual(runSubject.get("speedMiHrStackDecimal"), "05");
});

test('speedMiHrStackDecimal can be zero', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(1609.34)}); // 1.00 mi
...
test('speedMiHrStackDecimal setter changes speedMiHrStackDecimal', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", "9"); });
...
test('speedMiHrStackDecimal setter can handle floats', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(12000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", "8.2"); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimal"), "8");
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", 8.5); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimal"), "9");
});
...
test('speedMiHrStackDecimal setter handles negative values and changes speedMiHrStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedMiHr", "5.5"); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", -10); });
...
test('speedMiHrStackDecimal setter handles values over 99 and changes speedMiHrStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedMiHr", "5.5"); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", 100); });
  ...
  Ember.run(function(){ runSubject.set("speedMiHr", "5.5"); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", 123); });
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "5");
...
test('speedMiHrStackDecimal setter works with leading zeros', function(assert) {
  var runSubject = this.subject({timeSec :  new BigNumber(3600), lengthM :  new BigNumber(12000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", "09"); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimal"), "09");
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", "002"); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimal"), "0");
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimal", "009"); });
...
test('speedMiHrStackDecimal setter changes timeSec', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(3600), lengthM : new BigNumber(4828.032)});
  Ember.run(function(){ runSubject.setProperties({ speedMiHrStackMi : "1", speedMiHrStackDecimal : "5" }); });
...
test('speedMiHrStackMi and speedMiHrStackDecimal setter will define speedMiHr', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(7200), lengthM : new BigNumber(4400)});
  Ember.run(function(){ runSubject.setProperties({ speedMiHrStackMi : "12", speedMiHrStackDecimal : "05" }); });
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "12.05");

  Ember.run(function(){ runSubject.setProperties({ speedMiHrStackMi : "12", speedMiHrStackDecimal : "04" }); });
  assert.strictEqual(runSubject.get("speedMiHr").toString(), "12.04");

  Ember.run(function(){ runSubject.setProperties({ speedMiHrStackMi : "1", speedMiHrStackDecimal : "04" }); });
...
test('speedMiHrStackDecimalFixed property is calculated from speedMiHr and can round down', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedMiHr : new BigNumber(12.121) }); });
...
test('speedMiHrStackDecimalFixed property can round up', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedMiHr : new BigNumber(12.715) }); });
...
test('speedMiHrStackDecimalFixed has trailing zero(s)', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedMiHr : new BigNumber(0.5) }); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "50");
  Ember.run(function(){ runSubject.setProperties({ speedMiHr : new BigNumber(5) }); });
...
test('speedMiHrStackDecimalFixed supports leading zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedMiHr : new BigNumber(0.09) }); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "09");
  Ember.run(function(){ runSubject.setProperties({ speedMiHr : new BigNumber(0.005) }); });
...
test('speedMiHrStackDecimalFixed can be zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.setProperties({ speedMiHr : new BigNumber(99) }); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "00");
});

test('speedMiHrStackDecimalFixed setter changes speedMiHrStackDecimalFixed', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", "90"); });
...
test('speedMiHrStackDecimalFixed setter can handle floats', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", "8.2"); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "80");
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", 9.5); });
...
test('speedMiHrStackDecimalFixed setter also works with integer', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", 9); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "90");
});
...
test('speedMiHrStackDecimalFixed setter handles negative values and changes speedMiHrStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("speedMiHr", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", -10); });
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "4");
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed").toString(), "90");

  Ember.run(function(){ runSubject.set("speedMiHr", new BigNumber("5.0")); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", -1); });
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "4");
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed").toString(), "99");
});

test('speedMiHrStackDecimalFixed setter handles values over 99 and changes speedMiHrStackMi', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("speedMiHr", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", 100); });
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "6");
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed").toString(), "00");

  Ember.run(function(){ runSubject.set("speedMiHr", new BigNumber("5.5")); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", 123); });
  assert.strictEqual(runSubject.get("speedMiHrStackMi").toString(), "6");
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed").toString(), "23");
});

test('speedMiHrStackDecimalFixed setter works with leading zero', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber(1000)});
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", "09"); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "09");
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", "002"); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "00");
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", "009"); });
  assert.strictEqual(runSubject.get("speedMiHrStackDecimalFixed"), "01");
});

test('speedMiHrStackDecimalFixed setter changes speedMiHr', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.set("speedMiHr", "123"); });
  Ember.run(function(){ runSubject.set("speedMiHrStackDecimalFixed", "09"); });
...
test('speedMiHrStackMi and speedMiHrStackDecimalFixed setter will define speedMiHr', function(assert) {
  var runSubject = this.subject({timeMin : new BigNumber(1), lengthM : new BigNumber("1000")});
  Ember.run(function(){ runSubject.setProperties({ speedMiHrStackMi : "12", speedMiHrStackDecimalFixed : "9" }); });
...
test('speedKmHr accuracy edge case was fixed', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(30), lengthM : new BigNumber(23.4511)});
...
test('timeStackSecFixed setter changes timeMin', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber("3599.9999999999999999999999994")});
...
test('isInRange can deal with BigNumber input', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(30), lengthM : new BigNumber(23.4511)});
  assert.strictEqual(runSubject.isInRange(new BigNumber(12), new BigNumber(23.5)), true);
  assert.strictEqual(runSubject.isInRange(new BigNumber(32), new BigNumber(43)), false);
});

test('isInRange only returns true if the range is given in the right order', function(assert) {
  var runSubject = this.subject({timeSec : new BigNumber(30), lengthM : new BigNumber(12)});
  assert.strictEqual(runSubject.isInRange(new BigNumber(11), new BigNumber(13)), true);
  assert.strictEqual(runSubject.isInRange(new BigNumber(13), new BigNumber(11)), false);
