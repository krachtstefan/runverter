...
export default DS.Model.extend({
   ...
   */
  lastSplitDistance: computed(
    'run.lengthM',
    'splitDistance',
    'splitCountCeiled',
    function() {
      return this.get('run.lengthM').minus(
        this.get('splitDistance').times(this.get('splitCountCeiled').minus(1))
      );
    }
  ),
  ...
  turningPointSplit: computed('splitCountCeiled', function() {
    return this.get('splitCountCeiled')
      .dividedBy(2)
      .ceil();
  }),
   ...
   */
  lastSplitTime: computed(
    'run.timeSec',
    'splitTime',
    'splitCountCeiled',
    function() {
      return this.get('run.timeSec').minus(
        this.get('splitTime').times(this.get('splitCountCeiled').minus(1))
      );
    }
  ),
   ...
   */
  averagePaceFirstHalf: computed(
    'run.paceMinPerKmRaw',
    'splittingStrategy',
    function() {
      return this.get('run.paceMinPerKmRaw').plus(
        this.get('run.paceMinPerKmRaw')
          .times(this.get('splittingStrategy'))
          .dividedBy(100)
      );
    }
  ),
   ...
   * @return {BigNumber} average pace in min/km
   */
  averagePaceSecondHalf: computed(
    'run.paceMinPerKmRaw',
    'splittingStrategySecondHalf',
    function() {
      return this.get('run.paceMinPerKmRaw').plus(
        this.get('run.paceMinPerKmRaw')
          .times(this.get('splittingStrategySecondHalf'))
          .dividedBy(100)
      );
    }
  ),
  ...
  peakPaceValue: computed(
    ...
    function() {
      if (this.get('evenSlope') === false) {
        ...
        return this.get('splittingStrategy').greaterThan(0)
          ? this.get('averagePaceSecondHalf')
          : this.get('averagePaceFirstHalf');
      } else {
        // get the average pace of first or last meter of the race
        var x = this.get('splittingStrategy').greaterThan(0)
          ? new BigNumber(0)
          : this.get('run.lengthKmRaw');
  ...
  slope: computed(
    ...
    function() {
      ...
      var a = this.get('averagePaceFirstHalf').minus(
        this.get('averagePaceSecondHalf')
      );
      var b = this.get('run.lengthKmRaw')
        .dividedBy(4)
        .minus(
          this.get('run.lengthKmRaw')
            .dividedBy(4)
  ...
  shift: computed(
    ...
    function() {
      ...
      return this.get('averagePaceFirstHalf').minus(
        this.get('slope').times(this.get('run.lengthKmRaw').dividedBy(4))
      );
    }
  ),
  ...
  calculateSplits: function() {
    ...
    if (this.get('splitCountCeiled').greaterThan(1) === true) {
      for (
        let i = 1;
        this.get('splitCountCeiled').greaterThanOrEqualTo(i);
        i++
      ) {
        var thisSplitDistance = this.get('splitCountCeiled').equals(i)
          ? this.get('lastSplitDistance')
          : this.get('splitDistance'); // different length for last split

        var beforeTurningPoint = this.get(
          'turningPointSplit'
        ).greaterThanOrEqualTo(i); // are we in a split that is before the turning point
        var thisSplitTime = this.get('splitCountCeiled').equals(i)
          ? this.get('lastSplitTime')
          : this.get('splitTime'); // different time for last split

        var averagePaceCurrent;
        if (this.get('evenSlope') === true) {
          // get the average pace from the middle of the current split
          averagePaceCurrent = lengthMStack
            .plus(thisSplitDistance.dividedBy(2))
            .dividedBy(1000)
            .times(this.get('slope'))
            .plus(this.get('shift'));
        } else {
          averagePaceCurrent = beforeTurningPoint
            ? this.get('averagePaceFirstHalf')
            : this.get('averagePaceSecondHalf');
        }
        ...
        if (
          this.get('turningPointWithinSplit') === true &&
          this.get('turningPointSplit').equals(i) &&
          this.get('evenSlope') === false
        ) {
          var turningPointSplitDistance = this.get('turningPointM').minus(
            this.get('splitDistance').times(i - 1)
          );
          // determine the ratio between pre and post turning point distance
          var turningPointSplitRatio1 = turningPointSplitDistance
            .dividedBy(this.get('splitDistance'))
            .times(100);
          var turningPointSplitRatio2 = new BigNumber(100).minus(
            turningPointSplitRatio1
          );
          // determine the time of both splitting strategies
          var thisSplitTime1 = this.get('averagePaceFirstHalf')
            .times(60)
            .times(thisSplitDistance.dividedBy(1000));
          var thisSplitTime2 = this.get('averagePaceSecondHalf')
            .times(60)
            .times(thisSplitDistance.dividedBy(1000));
          // sum both times according to their ratio
          var time1 = thisSplitTime1
            .times(turningPointSplitRatio1)
            .dividedBy(100);
          var time2 = thisSplitTime2
            .times(turningPointSplitRatio2)
            .dividedBy(100);
          thisSplitTime = time1.plus(time2);
        } else {
          thisSplitTime = averagePaceCurrent
            .times(60)
            .times(thisSplitDistance.dividedBy(1000));
        }

        timeSecStack = timeSecStack.plus(thisSplitTime);
        var progressDistance = lengthMStack
          .dividedBy(this.get('run.lengthM'))
          .times(100);
        var progressTime = timeSecStack
          .dividedBy(this.get('run.timeSec'))
          .times(100);

        this.get('splits').push(
          Object.create({
