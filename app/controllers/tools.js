...
export default Controller.extend({
  ...
  toolsAvailable: ['pca', 'pc', 'lc', 'rp', 'sc'],
  ...
  openDonationMessage: function() {
    var donationMessage = this.get('notifications.content').filterBy(
      'message.string',
      this.get('i18n').t('flashMessages.donationMessage').string
    );
    if (donationMessage.length == 0) {
      this.get('notifications').success(
        '<span>' +
          this.get('i18n').t('flashMessages.donationMessage') +
          '</span>',
        { htmlContent: true }
      );
    }
  ...
  handleAchievedRunPersistence: observer(
    'model.prediction.achievedRun.lengthM',
    'model.prediction.achievedRun.timeSec',
    function() {
