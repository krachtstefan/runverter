...
export default Component.extend({
  ...
  runTempoMetrics: computed(
    'runTempoMetricsAvailable',
    'i18n.locale',
    'runTempoMetricsSelected',
    function() {
      var runTempoMetrics = [];
      var self = this;
  ...
  tooltipPaceMinPerKm: computed('run.paceMinPerKm', 'i18n.locale', function() {
    return (
      this.get('run.paceMinPerKm')
        .round(5)
        .toString()
        .replace('.', this.get('i18n').t('metrics.separator')) +
      ' ' +
      this.get('i18n').t('metrics.tempo.minkm')
    );
  }),

  tooltipPaceMinPerMi: computed('run.paceMinPerMi', 'i18n.locale', function() {
    return (
      this.get('run.paceMinPerMi')
        .round(5)
        .toString()
        .replace('.', this.get('i18n').t('metrics.separator')) +
      ' ' +
      this.get('i18n').t('metrics.tempo.minmi')
    );
  }),

  tooltipSpeedKmHr: computed('run.speedKmHr', 'i18n.locale', function() {
    return (
      this.get('run.speedKmHr')
        .round(5)
        .toString()
        .replace('.', this.get('i18n').t('metrics.separator')) +
      ' ' +
      this.get('i18n').t('metrics.tempo.kmh')
    );
  }),

  tooltipSpeedMiHr: computed('run.speedMiHr', 'i18n.locale', function() {
    return (
      this.get('run.speedMiHr')
        .round(5)
        .toString()
        .replace('.', this.get('i18n').t('metrics.separator')) +
      ' ' +
      this.get('i18n').t('metrics.tempo.mih')
    );
  }),
