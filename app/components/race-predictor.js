...
export default Component.extend({
  ...
  achievedTimesSuggestions: computed(
    'prediction.achievedRun.lengthM',
    'i18n.locale',
    function() {
      var self = this;
      return this.get('targetTimes.templates').filter(function(item) {
        return self
          .get('prediction.achievedRun.content')
          .isInRange(item.startM, item.endM);
      });
    }
  ),

  predictedTimesSuggestions: computed(
    'prediction.predictedRun.lengthM',
    'i18n.locale',
    function() {
      var self = this;
      return this.get('targetTimes.templates').filter(function(item) {
        return self
          .get('prediction.predictedRun.content')
          .isInRange(item.startM, item.endM);
      });
    }
  ),

  achievedRunLengthMetrics: computed(
    'runLengthMetricsAvailable',
    'i18n.locale',
    'achievedRunMetricsSelected',
    function() {
  ...
  predictedRunLengthMetrics: computed(
    'runLengthMetricsAvailable',
    'i18n.locale',
    'predictedRunMetricsSelected',
    function() {
      var runLengthMetrics = [];
      var self = this;
      this.get('runLengthMetricsAvailable').forEach(function(item) {
  ...
  importPredictedRun: function() {
    this.get('prediction.predictedRun').setProperties({
      lengthM: this.get('run.lengthM'),
      timeSec: this.get('run.timeSec')
    });
   ...
   */
  exportPredictedRun: observer(
    'prediction.predictedRun.lengthM',
    'prediction.predictedRun.timeSec',
    function() {
  ...
  displayPeterRiegelExlanation: observer(
    'prediction.peterRiegelMethodSuitable',
    function() {
      var self = this;
      if (
        this.get('prediction.peterRiegelMethodSuitable') === false &&
        this.get('settings.displayPeterRiegelExlanation') === true
      ) {
        this.set('settings.displayPeterRiegelExlanation', false);
        this.get('notifications').info(
          this.get('i18n').t('flashMessages.peterRiegelExlanation'),
          {
            onClick() {
  ...
  didRender: function() {
    ...
    run.scheduleOnce('afterRender', this, function() {
      var self = this;
      $('select.predictedRunLength')
        .selectOrDie({ customID: 'predictedRunLength' })
        .ready(function() {
      ...
      $('select.achievedRunLength')
        .selectOrDie({ customID: 'achievedRunLength' })
        .ready(function() {
  ...
  tooltipAchievedLengthKm: computed(
    'prediction.achievedRun.lengthKm',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.achievedRun.lengthKm')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
        this.get('i18n').t('metrics.distance.km')
      );
    }
  ),

  tooltipAchievedLengthMi: computed(
    'prediction.achievedRun.lengthMi',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.achievedRun.lengthMi')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
  ...
  tooltipAchievedTimeHr: computed(
    'prediction.achievedRun.timeHr',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.achievedRun.timeHr')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
        this.get('i18n').t('metrics.time.hr')
      );
    }
  ),

  tooltipAchievedTimeMin: computed(
    'prediction.achievedRun.timeMin',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.achievedRun.timeMin')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
        this.get('i18n').t('metrics.time.min')
      );
    }
  ),

  tooltipAchievedTimeSec: computed(
    'prediction.achievedRun.timeSec',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.achievedRun.timeSec')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
        this.get('i18n').t('metrics.time.sec')
      );
    }
  ),

  tooltipPredictedLengthKm: computed(
    'prediction.predictedRun.lengthKm',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.predictedRun.lengthKm')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
  ...
  tooltipPredictedLengthMi: computed(
    'prediction.predictedRun.lengthMi',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.predictedRun.lengthMi')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
        this.get('i18n').t('metrics.distance.mi')
      );
    }
  ),

  tooltipPredictedTimeHr: computed(
    'prediction.predictedRun.timeHr',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.predictedRun.timeHr')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
  ...
  tooltipPredictedTimeMin: computed(
    'prediction.predictedRun.timeMin',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.predictedRun.timeMin')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
  ...
  tooltipPredictedTimeSec: computed(
    'prediction.predictedRun.timeSec',
    'i18n.locale',
    function() {
      return (
        this.get('prediction.predictedRun.timeSec')
          .round(5)
          .toString()
          .replace('.', this.get('i18n').t('metrics.separator')) +
        ' ' +
  ...
  expertModeClass: computed('expertMode', function() {
    return this.get('expertMode') === true
      ? ''
      : 'uk-width-medium-4-6 uk-width-large-3-5';
  }),
  ...
  achievedRacePickerVisibleClass: computed(
    'achievedRacePickerVisible',
    'isTouchDevice',
    function() {
      return this.get('achievedRacePickerVisible') === true ||
        this.get('isTouchDevice') === true
        ? 'suggestSelectVisible'
        : 'suggestSelectInvisible';
    }
  ),

  achievedTimePickerVisibleClass: computed(
    'achievedTimePickerVisible',
    'isTouchDevice',
    function() {
      return this.get('achievedTimePickerVisible') === true ||
        this.get('isTouchDevice') === true
        ? 'suggestSelectVisible'
        : 'suggestSelectInvisible';
    }
  ...
  predictedRacePickerVisibleClass: computed(
    'predictedRacePickerVisible',
    'isTouchDevice',
    function() {
      return this.get('predictedRacePickerVisible') === true ||
        this.get('isTouchDevice') === true
        ? 'suggestSelectVisible'
        : 'suggestSelectInvisible';
    }
