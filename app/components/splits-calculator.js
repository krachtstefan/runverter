...
export default Component.extend({
  ...
  chartData: computed(
    'run{lengthM,timeSec}',
    'splitDistancesSelectedMeters',
    'splitStrategiesSelected',
    'evenSlopeSelected',
    function() {
      var data = {
        labels: this.get('run.splits.splits').map(function(data) {
          return data
            .get('run.lengthKm')
            .round(2)
            .toNumber();
        ...
        datasets: [
          {
            ...
            data: this.get('run.splits.splits').map(function(data) {
              return data.get('split.speedKmHr').toNumber();
            }),
            spanGaps: false
          }
        ]
  ...
  runLengthMetrics: computed(
    'runLengthMetricsAvailable',
    'i18n.locale',
    'runLengthMetricsSelected',
    function() {
  ...
  splitLengthMetrics: computed(
    'runLengthMetricsAvailable',
    'i18n.locale',
    'splitMetricsSelected',
    function() {
      var runLengthMetrics = [];
      var self = this;
      this.get('runLengthMetricsAvailable').forEach(function(item) {
  ...
  splitTempoMetrics: computed(
    'splitTempoMetricsAvailable',
    'i18n.locale',
    'splitTempoMetricsSelected',
    function() {
  ...
  splitStrategies: computed(
    'splitStrategiesAvailable',
    'i18n.locale',
    'splitStrategiesSelected',
    function() {
      var splitStrategies = [];
  ...
  didRender: function() {
    ...
    run.scheduleOnce('afterRender', this, function() {
      var self = this;
      $('select.runLength')
        .selectOrDie({ customID: 'runLength' })
        .ready(function() {
  ...
  calculateSplits: observer(
    ...
    function() {
      if (this.get('run.splits.content')) {
        this.set(
          'run.splits.splitDistance',
          this.get('splitDistancesSelectedMeters')
        );
        this.set(
          'run.splits.splittingStrategy',
          new BigNumber(this.get('splitStrategiesSelected'))
        );
  ...
  displayNoSplitsMessage: observer('run.splits.splits', function() {
    var noSplitsMessage = this.get('notifications.content').filterBy(
      'message.string',
      this.get('i18n').t('flashMessages.noSplits').string
    );
    if (this.get('run.splits.splits.length') === 0) {
      if (noSplitsMessage.length == 0) {
        this.get('notifications').info(
          this.get('i18n').t('flashMessages.noSplits')
        );
      }
    } else {
  ...
  splitDistancesPossible: computed(
    'run.lengthM',
    'splitMetricsSelected',
    'splitDistancesSelected',
    function() {
      var self = this;
      ...
      this.get('splitDistancesAvailable').forEach(function(item, index) {
        ...
        numberOfPossibleSplits = self
          .get('run.lengthM')
          .dividedBy(splitDistanceM)
          .ceil()
          .toString();
      ...
      if (
        splitDistancesPossible.indexOf(this.get('splitDistancesSelected')) == -1
      ) {
  ...
  splitDistancesSelectedMeters: computed(
    'splitDistancesSelected',
    'splitMetricsSelected',
    function() {
      if (this.get('splitMetricsSelected') === 'mi') {
        return new BigNumber(this.get('splitDistancesSelected')).times(
          this.get('run.miToM')
        );
      } else {
  ...
  racePickerVisibleClass: computed(
    'racePickerVisible',
    'isTouchDevice',
    function() {
      return this.get('racePickerVisible') === true ||
  ...
  timePickerVisibleClass: computed(
    'timePickerVisible',
    'isTouchDevice',
    function() {
      return this.get('timePickerVisible') === true ||
        this.get('isTouchDevice') === true
        ? 'suggestSelectVisible'
        : 'suggestSelectInvisible';
    }
  ...
  evenSlopeVisibleClass: computed('evenSlopeVisible', function() {
    return this.get('evenSlopeVisible') === true
      ? 'evenSlopeVisible'
      : 'evenSlopeInvisible';
  }),
